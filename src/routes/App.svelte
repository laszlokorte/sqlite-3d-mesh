<script>
	...
	onMount(async () => {
		...
		let sqlstr = [
			...
			//"CREATE VIEW projected_face AS SELECT vp.id AS viewport_id, v.vertex_id AS vertex_id, v.camera_id AS camera_id, vp.width * (0.5 + 0.5*v.x) AS x, vp.height * (0.5 + 0.5*v.y) AS y, v.z AS z, v.w AS w FROM ndc_vertex v INNER JOIN viewport vp ON vp.camera_id = v.camera_id WHERE abs(v.x) < 1 AND  abs(v.y) < 1  AND abs(v.z) < 1",
		...
		statements = {
			...
			faceSelect: db.prepare(
				"SELECT face.id AS face_id, v.vertex_id AS vertex_id, v.x AS x,v.y AS y, v.z AS z FROM face INNER JOIN face_vertex fv ON fv.face_id = face.id INNER JOIN projected_vertex v ON v.vertex_id = fv.vertex_id WHERE v.viewport_id = $viewport ORDER BY fv.sort ASC",
			),
		};
	...
	function update(db) {
		if (db) {
			...
			{
				const rows = {};
				statements.faceSelect.bind({ $viewport: 1 });
